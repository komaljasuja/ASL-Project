...
class Application:
    ...
    def __init__(self):
        ...
        self.translate.place(x=805, y=630)
        self.translate.config(text="Translate", font=("Courier", 14), wraplength=120, command=self.translate_fun)
        ...
        #def in main to get the list of word - sreeni
        english_words = self.load_words()
        self.sorted_eng_words = sorted(english_words)
        ...
        #for emotions
        self.emotions = ['SAD' , 'HAPPY', 'SURPRISED']

        with open('./smodel', 'rb') as f:
            self.facemodel = pickle.load(f)
    ...
    def video_loop(self):
        try:
            ...
            #code for face - sreeni
            
            face_landmarks = get_face_landmarks(frame, draw=True, static_image_mode=False)
            if (len(face_landmarks) > 1400):
                output = self.facemodel.predict([face_landmarks])
                #print(output)
                #used cv2image to get the same image on both windows but the 
                #image quality is not good.
                
                cv2.putText(frame,
                            self.emotions[int(output[0])],
                        (10, frame.shape[0] - 1),
                ...
                cv2.waitKey(25)
            #completed the face integration
    ...
    #this function is to load the word so that we can predict the complete word - sreeni
    def load_words(self):
        with open('./word_data/words_alpha.txt') as word_file:
    ...
    def translate_fun(self):
        translation = self.translator.translate(self.str, dest = "es")  #  dest="fr") #   ("Hello World" , dest="es")   #("Hola Mundo", dest="en")
        print(f"{translation.origin} ({translation.src}) --> {translation.text} ({translation.dest})")
        print(f"{translation.origin}")
        print(f"{translation.text}")
        self.str = self.str + "    " +translation.text
    ...
    def predict(self, test_image):
        ...
        if len(self.str.strip())!=0:
            ...
            if len(word.strip())!=0:
                ...
                print("--------strvar",strvar)
                matching_words = []
                for word in self.sorted_eng_words:
                ...
                lenn = len(matching_words)
                if lenn >= 4:
                    self.word4 =  matching_words[3] #ddd.suggest(word)[3]
                    print(matching_words[3])

                if lenn >= 3:
                    self.word3 = matching_words[2]  #ddd.suggest(word)[2]

                if lenn >= 2:
                    self.word2 =matching_words[1] # ddd.suggest(word)[1]

                if lenn >= 1:
                    self.word1 = matching_words [0]  #ddd.suggest(word)[0]
            else:
                self.word1 = " "
                self.word2 = " "
                self.word3 = " "
